...
#define TRANSFER 400
#define INTAKE_SHOOT_FAR 0
#define INTAKE_SHOOT_CLOSE 0
...
#define PUSHER_OUT 2170
	...
	void advancedShoot(void)
	{
		...
		case 50:
			...
			shooter1->Set(0.f);
			shooter2->Set(0.f);
			if(m_Gamepad->GetRawButton(GP_B) && timer->Get() > 1.5)
			{
				shooterState = 60;
				timer->Stop();
				timer->Reset();
		...
		case 60:
			...
			if(m_Gamepad->GetPOV() == GP_UP)
				shooterState = 20;
			if(m_Gamepad->GetRawButton(GP_DOWN))
				shooterState = 10;
